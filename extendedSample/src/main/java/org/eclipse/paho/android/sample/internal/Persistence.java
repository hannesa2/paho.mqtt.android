...
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
    ...
    /* (non-Javadoc)
     * @see org.eclipse.paho.android.service.database.sqlite.SQLiteOpenHelper#onCreate(org.eclipse.paho.android.service.database.sqlite
     * .SQLiteDatabase)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see org.eclipse.paho.android.service.database.sqlite.SQLiteOpenHelper#onUpgrade(org.eclipse.paho.android.service.database.sqlite
    ...
    /*
     * (non-Javadoc)
     * @see org.eclipse.paho.android.service.database.sqlite.SQLiteOpenHelper#onDowngrade(org.eclipse.paho.android.service.database.sqlite
     * .SQLiteDatabase, int, int)
     */
    @Override
    ...
    public List<Connection> restoreConnections(Context context) throws PersistenceException {
        ...
        Cursor cursor = db.query(TABLE_CONNECTIONS, connectionColumns, null, null, null, null, sort);
        ArrayList<Connection> list = new ArrayList<Connection>(cursor.getCount());
        Connection connection;
        for (int i = 0; i < cursor.getCount(); i++) {
            if (!cursor.moveToNext()) { //move to the next item throw persistence exception, if it fails
                throw new PersistenceException("Failed restoring connection - count: " + cursor.getCount() + "loop iteration: " + i);
            }
            //get data from cursor
            Long id = cursor.getLong(cursor.getColumnIndexOrThrow(_ID));
            //basic client information
            String clientHandle = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_CLIENT_HANDLE));
            String host = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_HOST));
            String clientID = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_client_ID));
            int port = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_port));
            ...
            int qos = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_QOS));
            int keepAlive = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_KEEP_ALIVE));
